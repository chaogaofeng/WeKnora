...
func (h *Handler) completeAssistantMessage(ctx context.Context, assistantMessage *types.Message, userQuery string, feedbackKBIDs ...[]string) {
	...
	// question on the agent. Skipped for non-agent replies
	...
	// Long-term memory write for this finished turn (main's feature). The user
	// message ID is not plumbed into this entry point because the OpenAI-compatible
	// and RAG paths call it via the dev signature, so provenance is left empty.
	if userQuery != "" {
