...
func (h *KnowledgeHandler) CreateKnowledgeFromFile(c *gin.Context) {
	...
	// M4: optional target folder. When set, dedup is scoped to that folder and
	// the created file is assigned to it directly (so the same file may exist
...
func (h *KnowledgeHandler) ListKnowledge(c *gin.Context) {
	...
	filter := types.KnowledgeListFilter{
		...
		FolderIDs:   parseCommaSeparatedTagIDs(c.Query("folder_ids")),
	}
	...
	logger.Infof(
		ctx,
		"Retrieving knowledge list under knowledge base, kb_id=%s tag_ids=%s folder_ids=%s keyword=%s file_type=%s parse_status=%s source=%s start_time=%s end_time=%s folder_path=%s folder_scope=%s page=%d page_size=%d effectiveTenantID=%d",
		secutils.SanitizeForLog(kbID),
		secutils.SanitizeForLog(strings.Join(filter.TagIDs, ",")),
		secutils.SanitizeForLog(strings.Join(filter.FolderIDs, ",")),
