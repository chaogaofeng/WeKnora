...
type Message struct {
	...
	// PushedKnowledgeIDs holds the knowledge_ids that were pushed by the
	// push_files tool in this message. RecordReferenceEvents uses it to tag the
	// corresponding reference events as reference_type='push'. Only the Agent
	// path populates this field. Serialized as a JSON array of strings.
	PushedKnowledgeIDs JSON `json:"pushed_knowledge_ids,omitempty" gorm:"type:json;column:pushed_knowledge_ids;not null;default:'[]'"`
...
func (m *Message) BeforeCreate(tx *gorm.DB) (err error) {
	...
	if m.PushedKnowledgeIDs == nil {
		m.PushedKnowledgeIDs = JSON("[]")
	}
	if m.AgentSteps == nil {
